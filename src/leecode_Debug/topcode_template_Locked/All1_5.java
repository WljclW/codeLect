...
//     * 从前序 和 中序 构造出二叉树*/
    ...
    public String reverseWords(String s) {
        /*step1：去除字符串首尾的空格。（隐含的一个预处理后特点：第一次进入while的时候，i位置的字符一定不是空格，是正常的字母）*/
        s = s.trim();
        /*step2：声明几个标志变量*/
        StringBuilder res = new StringBuilder();
        int i = s.length()-1,j = i; //指针i 和 指针j 都指向最后一个单词的最后一个字符
        /*stp3：while循环依次研究每一个位置。其实每一次while循环中做三件事————
                1. 首先 让指针i倒着走，来到第一个空格的位置；————此时指针i相当于“倒着走过了最后一个单词”
                2. 反转最后一个单词添加进答案—————解释：经过1后i位置是空格，因此最后一个单词即为“substring(i+1,j+1)”
                3. 继续 让i倒着走，走到第一次碰到不是空格的位置；————解释：此时该位置是“目前没有研究的最后一个单词的最后
             一个字符”
                4. 将i的值赋值给j（相当于j重新标记了最后一个单词的最后一个位置），循环1~3————解释：此时j位置就是“目前还
             没有研究的最后一个单词 的最后一个字符位置”
        */
        while (i>=0){
