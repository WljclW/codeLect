...
public class _14DP {
    ...
    * */
    ...
    public int climbStairs2(int n) {
        if(n==1) return 1;
        if(n==2) return 2;
        int p1 = 1,p2 = 2; /*1个台阶、2个台阶是base case*/
        for(int i=3;i<=n;i++){
            int tmp = p2;
            p2 = p1+p2;
            p1 = tmp;
    ...
    * */
    /**
     【建议的解法】 解法 generate_own 的思想直观，思路简单就是直接模拟；
                   解法 generate 是最优的，不用过多的特殊考虑，泛化更高建议使用 generate 方法。
     */
    public List<List<Integer>> generate(int numRows) {
        ...
        for (int i = 3; i <= numRows; i++) { /*外层for循环：计算第i行的List*/
            LinkedList<Integer> ele = new LinkedList<>();
            List<Integer> last = res.getLast();
            for (int j = 0; j < i; j++) { /*内层循环：依次计算出第i行j位置的值，j的范围————[0,i-1]*/
                /**这种方法的好处：不用特殊考虑每一行的首、尾位置*/
                /*计算上一行左边的那个数：如果越界了，取0*/
                int leftVal = (j - 1 < 0) ? 0 : last.get(j - 1);
                /*计算上一行index相同的那个数：如果越界了，取0*/
    ...
    /*下面是自己的写法*/
    public List<List<Integer>> generate_own(int numRows) {
        LinkedList<List<Integer>> res = new LinkedList<>();
        /*step 1. 特殊情况的考虑*/
    ...
    * 用字典中出现的一个或多个单词拼接出 s 则返回 true。
    ...
    /**
     * 【关键】dp[i]：0~i-1的子串是否能由wordDict的数个字符串组成，也可以说成 串s中的前i个字符的子串能否由wordDict中的
     *      词拼出来。
     */
        ...
        for (int i = 1; i < s.length() + 1; i++) {
            for (int j = 0; j < i; j++) { //j<i，因此j其实并不能取到s.length()，但是i可以取到。所以这种时候能枚举完整个s
                ...
                if (dp[j] && set.contains(s.substring(j, i))) { /**【说明】根据双层for的条件，i的最大值s.length()，j<i因此j的最大值是s.length()-1,因此subString是不会越界的*/
                    dp[i] = true;
                    break; //这里加break会继续研究s串的下一个位置（因为一旦一个位置找到一种方案其实就没必要再研究这个位置了），不加也没啥问题
                }
            }
        }
        return dp[s.length()];
    ...
    * 序。例如，[3,6,2,7] 是数组 [0,3,1,6,2,2,7] 的子序列。*/
    /**
     * 【强烈建议】使用 lengthOfLIS1 ————解法2
     * 【解法1的思路】定义 dp[i] 表示以 nums[i] 结尾的最长递增子序列长度，依次求解每个dp[i]的值。
            对于每个 i，遍历 0 ~ i-1 的所有 j：
                如果 nums[j] < nums[i]，则nums[i]可以接在 nums[j] 后面 形成一个更长的递增子序列，更
            新 dp[i] = max(dp[i], dp[j] + 1)。最终答案为所有 dp[i] 中的最大值。
      【解法2的思路】维护一个数组 tails，tails[i] 表示长度为 i+1 的递增子序列的的末尾元素 的最小值。
            遍历 nums，对每个元素二分查找 tails 中第一个 ≥ 当前值的位置替换（本质：在tails中查找某
        数的插入位置，见 35题）；若找不到，则说明当前元素比 tails的所有元素都大，直接追加到 tails。
        最后，tails 的长度就是最长递增子序列的长度。
     * */
    /*解法1：朴素的做法————依次求解以nums[i]结尾的最长递增子序列 的长度
    * dp[i]：表示以第i个元素结尾的最长递增子序列有多长*/
    public int lengthOfLIS(int[] nums) {
        int[] dp = new int[nums.length];
        dp[0] = 1;
        int res = 1;
        for (int i=1;i<nums.length;i++) { /*外层for循环的含义：求解以 nums[i] 结尾的最长递增子序列的长度*/
            for (int j = 0; j < i; j++) { /*内层for循环的含义：从0依次枚举，看看 nums[i]能不能放到nums[j]后面形成一个更长的递增子序列*/
                if (nums[j] < nums[i]) {
                    dp[i] = Math.max(1, dp[j] + 1);
                }
            }
            res = Math.max(dp[i], res); //经过内层 for 循环后，dp[i]的值已经计算出来了，这里看看是不是更新全局答案
        }
        return res;
    }
    ...
    【理解一个关键点】要想让某一个子序列尽可能的长，则子序列最后一个数应该尽可能的小
    * 看K神的讲解：https://leetcode.cn/problems/longest-increasing-subsequence/，K神的写法是左闭右开的写法
    * 此时tails[i]的定义如下：
    *       tails[i]的值为num代表————数组nums中 以num结尾的最长递增子序列长度为i+1。
    * */
    /**
     * 【关键】把数字num放在以它结尾的最长递增序列处。对于每一个数研究应该放在哪一个位置。
     * 【与dp动规最根本的区别】
     *      dp动规的方法中数组dp的含义：每一个元素的含义是以该位置元素结尾的最长递增子序列的长度。比如：dp[i]表示以nums[i]结尾的最长递
     *   增子序列的长度是dp[i]。
     *      贪心的方法中数组tails的含义：每一个元素代表着该长度递增子序列的所有例子中，最后一个数的最小值。比如：dp[i] = m的含义是长度
     *   为i+1的递增子序列的最后一个数 最小可能的值是m。（这种含义保证了对于任何一个数random，在tails数组中找到它应该插入的位置，就知道
     *   到目前索引为止，以它结尾的最长递增子序列的长度是多少）
    ...
    public int lengthOfLIS1(int[] nums) {
        ...
        for (int num : nums) {
            ...
            tails[left] = num;
            /*step4：如果num插在了索引为size的位置（实际上这个位置之前还没有放过数，如果是换做35题的话就相当
            于本来是size大小的数组，下标0~size-1，但是计算出的插入位置是在size（等价于35题得到 nums.length），
            相当于放在数组的最后），则更新size*/
            if (left == size)
    ...
    * 少包含一个数字），并返回该子数组所对应的乘积。
    测试用例的答案是一个 32-位 整数。*/
    /**
     【建议】建议使用 maxProduct2 或者 maxProduct3，条理清晰。。。最推荐的是 maxProduct2！！
     【注意】
     *      求的是最大值，但是不能仅仅记录之前子数组的最大值。比如"-2,3,-4"的最大值应该是24，如果
     *  仅仅选取最大值则遍历到元素3的时候，会选择单独成一个子数组，这样是不对的。应该到任何一个位置，
     *  都应该记录 以当前位置i结尾的子数组的最大值、最小值...
    ...
    public int maxProduct2(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) { /**err：i要从1开始，因为res的初始值已经是nums[0]了*/
            ...
            /**err：下面的操作是直接赋值，而不是取最值！！
               原因：每一个位置研究完，minPre、maxPre必须是当前位置 nums[i] 结尾的子数组的乘积最大/最小
             值。要忘记之前的，直接覆盖*/
            minPre = curMin;
            maxPre = curMax;
...
//    }
    ...
    * 长度。*/
     ...
     *     2. 从右到左遍历时，正好相反。如果某一时刻left>right，则将left以及right重置为0（前提：我们是倒着遍历，因为left>right的
     *  时候，继续遍历肯定不能扩大有效括号的长度————如果碰到右括号肯定不行，会有”))((((“的形式，但是并不是有效的括号串；碰到左括号的
     *  话，左括号的数量更大了，因此也不符合。结论：left>right后不能在扩大有效括号的长度了）
     * 【疑问】为什么需要两次遍历？？
    ...
    public int longestValidParentheses_point(String s) {
        ...
        left = right = 0; /**err：left和right恢复初始值，少不了。。否则出事的测试用例"(()"就返回了错误的结果4*/
