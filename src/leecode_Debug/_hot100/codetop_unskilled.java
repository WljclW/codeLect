...
//            sb.append(c).append('#');   /*等价于在最后添加 字符'#'*/
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (p[i] > maxLen) {
                 ...
                 */
                maxLen = p[i]; /**回文半径就是“最长回文子串”的长度，str的回文子串长度都是奇数，但是回文半径可能是奇数、也可能是偶数*/
                /**❗❗❗❗❗❗❗
...
//        k %= nums.length;  //错误的，不能有
    ...
    private int findKthLargest_best(int[] nums, int left, int right, int index) {
        ...
        * */
        ...
        if (cur[0]>index){ //说明要查找的索引出现在“等于区域”的左边，因此去左边找
            return findKthLargest_best(nums,left,cur[0]-1,index);
        } else if (cur[1]<index) { //说明要查找的索引出现在“等于区域”的右边，因此需要去右边找
    ...
    /**
        1. 这里的 partion_best方法 其实就是"颜色分类"的原版代码。————
            left指针：第一个等于pivot的值在哪个位置。（这个值一定存在，因为pivot就是从数组
        中选出来的）
            right指针：下一次碰到大于pivot的时候，元素应该放置的位置。因为while循环的循环条
        件是“while (cur<=right)”，因此出了while循环时，right指针其实就是pivot最后一次出
        现的位置
        2. 返回值应该是什么？
            首先，left代表的是第一个等于 pivot 元素的位置；
            其次，由于while循环的条件是“cur<=right”，因此在等于的时候也会继续研究。①如果这个
        小于pivot，则执行后left+1，cur+1，但是其实最后一个等于pivot的位置是cur-1，即right；
        ②如果这个值等于pivot，cur+1，但是由于这个位置已经研究过了，因此最后一次出现的位置依然是
        cur-1，即right；③如果这个值大于pivot，则cur不动，right-1，此时会发现cur>right并且right
        位置的值已经研究过了，因此最后出现的位置也是right。
            【注】由于这个题 pivot 就是从数组中选出来的，因此这个值必然存在，也即必然存在闭区间[m,n],
        使得这个区间内的所有元素等于pivot
     */
    private int[] partion_best(int[] nums, int left, int right, int pivot) {
        int cur = left;
        while (cur<=right){ /**注意：这里必须带等于*/
            if (nums[cur]<pivot){
                swap11(nums,left++,cur++);
            } else if (nums[cur] > pivot) {
                swap11(nums,cur,right--); //到了这里会发现：right其实就是第一个大于pivot值的位置
            }else {
                cur++;
            }
        }
        /**这里的返回值，正确的。这是关键，保证“闭区间[left,right] 内的元素都是等于flag的”*/
