...
public class _01hash {
    ...
    * 给你一个字符串数组，请你将 字母异位词 组合在一起。可以按任意顺序返回结果列表。
    ...
    /**
     * 【推荐的写法】强烈推荐写法 groupAnagrams_best。
     *      两种写法的区别？
     *          相同点：使用map<String,List<String>>，来把应该在一个组的string存储到某一个key
     *      对应的value中
     *          不同点：根据字符串构建key的思路不同。一种是无脑的排序字符串的字符，最小的时间复杂
     *      度O(n*logn)；最好的做法是统计每种字符的数量，使用一个26长度的数组，然后使用数组构造出
     *      字符串，将构造出的字符串作为map的key
     * 【解题思路】
