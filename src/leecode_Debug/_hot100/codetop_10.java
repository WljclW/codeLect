...
//    }
     ...
     */
    public ListNode oddEvenList__(ListNode head) {
        if(head==null||head.next==null) return head;
...
//        root.right = left;
    ...
    public String removeDuplicateLetters_1(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (isExit[c-'a']){ /**如果栈中已经出现了字符c，就结束本次循环*/
                continue;
            }
    ...
    /*其他的写法*/
    public String removeDuplicateLetters_(String s) {
        int[] flags = new int[26];
        boolean[] used = new boolean[26];
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
...
给定两个整数数组 gas 和 cost ，如果你可以按顺序绕环路行驶一周，则返回出发时加油站的编号，否则返回 -1 。如果存在解，则 保证 它是 唯一 的。
    ...
    /*解法1：递归方法的实现。
        ...
        空间复杂度：O(n)（递归栈）
    */
    public int iceBreakingGame_(int n, int m) {
        if (n == 1) return 0;
        return (iceBreakingGame_(n - 1, m) + m) % n;
    }


    /*写法2：迭代版本
        当 i = 1，只有一个人，结果显然是 0；每增加一个人，相当于把原来结果“向后移动 m 位”。
    * */
    public int iceBreakingGame(int num, int target) {
        ...
        /*从两个数开始推，一直推到第num个数的答案。
            ...
            ......依次类推
        * */
        for (int i = 2; i <= num; i++) { /**err：由于只有一个人的时候结果是0即res，因此接下来从两个人开始研究————即i从2开始*/
            res = (res + target) % i; /**err：由于报数是从1开始，因此往后数target就是下一个排除的数！！使用”(res+1+target)%i“是错误的，这个语句的含义是：从0开始报数*/
        }
        return res;
    }
     ...
     *       ①如果high小于0，直接返回false。（表示最多未匹配的左括号是-high个，不可能哇。。。因此high--只发
     * 生在碰到‘)’的时候，high<0说明')'太多了，即使*全部当作'('也不够')'的数量，返回false）
...
//                if (i == j) continue; //如果索引相等，不允许
    ...
    /**
     *【时常错误】
     *      1. 每一轮最开始的数start，以及最后计算对应的数num都必须使用long类型来存放，否则会有报错
     *      2. 求解”第N个数字“再哪一个数的时候，需要对 除了while循环的n减1计算。详细解释————
     *      比如：n = 11。对应的数字应该是0。。代码的执行流程，除了while循环发现n变为2，start变为10，此
     *   时计算对应的哪一个数应该使用”num = start + (n-1)/digit“即”10+(2-1)/2“发现num就是10，然
     *   后拿出具体的索引”index = (n-1)%digit“即”(2-1)%1=1“，因此拿出10的index=1的数字是0.
     *      反之，如果不减1，”num = start + (n)/digit“即”num=10+2/2“得到num=11，但是其实”第10个数
     *   字“应该是在10这个数中，很明显不减1是错误的
    ...
    public int findNthDigit_own(int n) {
        ...
        /**下面具体的拿出某一个位置数的时候需要使用“-'0'”，否则会出现如下的错误
                         输入
                         n =
                         3
                         输出
                         51   //某一个位置的数字很明显不可能超过10，51明显就是错误的
                         预期结果
                         3
         */
     ...
     */
    /**
     * 【建议的解法】使用 triangleNumber。这个题最最关键的是“倒着枚举最大的边”，此时可以使用“较小两
     *      边和一定要大于第三边”来快速计算有多少种。
     * 【关键】————很关键！
    ...
    public int triangleNumber(int[] nums) {
        ...
        for (int k = nums.length-1; k >= 2; k--) { /**err：k=2的时候也需要执行循环。k=2时，i=0,j=1*/
            ...
            while (left<right){
                /*1. 如果当前三个位置可构成三角形，则 计算数量 并且 尝试缩小right————if块干的事
                  2. 如果当前三个位置不构成三角形，则尝试更新left。因为right是k-1，已经不能再大
                    了————else块干的事
                */
                if (nums[left]+nums[right]>nums[k]){
                    /*  想象成right不变，left可以选择left、left+1、left+2...right-2、right-1（这些值作为left也必然满足
                    “nums[left]+nums[right]>nums[k]”）。因此选择后下一行改变right就不会有重复的组合
...
//    }
     ...
     */
    /**
     *【建议】建议使用解法 longestOnes 或者 longestOnes_01
    ...
    public int longestOnes_01(int[] nums, int k) {
        int left = 0,right = 0; /*left：窗口的左边界。right：窗口的右边界*/
        int zeroCount = 0;
        int res =0;
        while (right<nums.length){
            ...
            while (zeroCount>k){ /**窗口内0的数量太多了，移动左边界*/
        ...
        *   ③其他的情况需要入栈*/
        for (int i = 0; i < split.length; i++) {
            if (split[i].equals("")||split[i].equals(".")){
                ...
                /**err：弹出元素时必须判空，否则提交报错"java.util.EmptyStackException"；
                    并且这里“if中的条件”不能集成在 else if的条件中
                 */
...
返回你为了学完所有课程所安排的学习顺序。可能会有多个正确的顺序，你只要返回 任意一种 就可以了。如果不可能完成所有课程，返回 一个空数组 。
    ...
    public int numTrees(int n) {
        int[] dp = new int[n + 1];
        /**err：0个节点的“二叉搜索树”有1种
            必须要初始化dp[0]，否则初始的测试用例有错。。。比如3个节点，左子树两个、右子树没节点，此时有dp[2]*dp[0]种
         可能，如果不初始化dp[0]得到的就是0
         */
        ...
        for (int i = 2; i < n+1; i++) { /*依次求解：i个节点组成的二叉搜索树有多少种*/
            for (int j = 1; j <= i; j++) { /*求解方式：分别求出以“第j个节点为根”的搜索树有多少种，加和就是dp[i]*/
                /**【注】这是一个累加的过程。分别是第1个节点作为根的方案数、第2个节点作为根的方案数.....因此总方案数是这些的加和。
                 ...
                 2. ”i个节点组成的二叉搜索树数量“等于 以第1个节点作为根节点的二叉搜索树数量 + 以第2个节点作为根节点的二叉搜索树数
                 量 .... + 以第i个节点作为根节点的二叉搜索树数量
                 */
                dp[i] += dp[j-1]*dp[i-j]; /*求出以j（第j个节点）为根的二叉搜索树有多少种？左边有j-1个节点；右边有i-j个节点。（左右子树一共i个节点正确）*/
            }
        }
        return dp[n];
    }
