...
//        PriorityQueue<int[]> queue = new PriorityQueue<>(Comparator.comparingInt(o -> o[1]));
    ...
    */
     ...
     *      理论上来讲，时间复杂度这种写法基本就是最优解了。。。。其他的写法仅仅可能在常数时间稍微优化一下，可优化的空间就在
     *   于下面的写法不管三七二十一，都会进行一次入优先级队列、一次出优先级队列；优化的时候无非就是考虑特殊情况的时候其实不用
     *   这样“南辕北辙”的做法
     *【时间复杂度】
     *      插入的时间复杂度 logN，如何理解？
     *      首先，插入的步骤：插入步骤：
                插入元素：新元素通常被放在堆的末尾。
                上浮操作：接下来，你要检查新元素与它的父节点的大小关系。如果堆的性质被破坏（例如，对于大顶堆，新元素大于父节点），你需要交换它们的位置。
                重复步骤2：继续上浮新元素直到堆的性质满足。
            那时间复杂度如何考虑？
                每一次交换的时间复杂度是 O(1)。最大上浮次数是堆的高度。一个完全二叉树的高度是 O(log n)，因此，最坏情况下需要执行 O(log n) 次交换。
     * [说明]：
        ...
        public MedianFinder_1() {
            /**min：存放较小的那一半。要求：能快速拿出小数中的最大值；
               max：存放较大的那一半。要求：能快速拿出大数中的最小值
                 由于优先级队列默认是"小根堆（升序的）————“小根堆”堆顶是最小值"，但是min要快速拿出最大值，因此需
             要指定排序规则。
             * */
    ...
    /*1. 跟 MedianFinder_1 是一样的思路，看 MedianFinder_1 即可，不用看下面的写法！
      2. 下面写法的目的：说明在添加元素时的一个错误做法。
        结论：不能简单的根据两个队列的大小来放置元素，这样不能保证全局是有序的。。。
        全局有序的判定标准：max(存较小数的队列)<=min(存较大数的队列)。即 较小数的最大值 不
      大于 较大数的最小值
     * */
    class MedianFinder {
