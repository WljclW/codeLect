...
public class _13tanXin {
    ...
    * */
    /**
     【推荐的写法】
            1. while循环的写法见 canJump
            2. for循环的写法见 canJump_for_2
     */
...
//            bound = Math.max(bound, i + nums[i]);
    ...
    public boolean canJump_for_2(int[] nums) {
        int right = 0;
        /**i满足什么条件：（1）小于 nums.length；（2）不超过当前能到的最远边界————即right。。。直接把这些条件
         写在for循环的条件中
         */
    ...
    * */
     ...
     * 【建议】建议使用下面的写法 partitionLabels 或者 partitionLabels2.
    ...
    public List<Integer> partitionLabels(String s) {
        ...
        /*step2：循环遍历，求满足要求的子串*/
        int start = 0 /*start 代表当前组的左边界（包含该位置）*/, end = flags[s.charAt(0) - 'a']; /*end 代表当前组的右边界（包含该位置）。end初始化为0也没问题。*/
        for (int i = 0; i < s.length(); i++) {
            ...
            if (i == end) {
                res.add(end - start + 1);
                start = i+1; //更新start的值。下一组的第一个位置就是 i+1，因此 start = i+1
            }
        }
        return res;
    }
