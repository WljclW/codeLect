...
*                                     if (nums[mid]>target){
    ...
    * */
    /**
     【关键总结】
            1. 这个题 mid 位置的数 和 那个边界判断 比较合理？？
                答：都可以。原因如下：
                （1）如果 nums[mid] >= nums[left]，则 mid 的左边必然是有序的；
                （2）如果 nums[mid] >= nums[right]，则 mid 的左边必然是有序的
     */
    /**
     ...
     *      能省略，否则测试用例“[3,1]”结果是错误的；与183是有区别的，183 寻找旋转数组的最小值 是和nums[right]来
     *      比较的
     * 【关键（理解二分法的本质）】理解二分查找的本质：是能根据一个条件排除一半的区间，每一步我们都知道下一步应
     *      该去哪一半查找
     ...
     *      在"寻找旋转数组最小值"中，每一步的nums[mid]需要和nums[right]比较。只要nums[mid]>=
     *  nums[right]，则数组必然分为两半有序并且mid是在前一段中，此时最小值位置一定是在mid的右边
     *  并且不包括mid位置；否则nums[mid]<nums[right]的时候数组可能是整体有序也可能是分两段有序，
    ...
    * */
    public int search_another(int[] nums, int target) {
        ...
        while (left<=right){
             ...
             */
            if (nums[mid]>nums[right]){ /**说明：这里带等于也是对的*/
...
你必须设计一个时间复杂度为 O(log n) 的算法解决此问题。
    * */
    /**这个题其实“最好和右边界的数比较，判断下一步去那半边继续找”。为什么？？
        原因分析： 这个题与最左边的比较不合适，原因————
                比如如果nums[mid]>nums[left]，接下来应该取那一边查找呢？？此时不一定的，没有一个明确的方向，如果使
            用“mid 位置的数 和 left 位置的数比较”的话必须添加额外的判断！！！举个例子：
                ...
                综上，这样的比较并没有一个明确的方向，因此尽量不要这么比较....
        结论：尽量和 右边界的数 进行比较判断接下来的方向！！！
     */
    /**
     * 【解题关键】
     *      1. 必须是和数组中的最后一个元素比！！！！原因：比之后能知道 应该向左还是向右继续寻
     * 找，才能找到最小值。。即和"最后一个元素"比能把搜寻区间减半。解释如下：
     *          ①如果nums[mid]大于等于"最后一个数"，则这个数组一定是分为两段有序，并且最小值是在
     *      mid的右边，因此这种情况下更新left为mid+1；
     *          ②如果nums[mid]小于"最后一个数"，则这个数组不一定是分为一段还是两段。但是最小值一
     *      定是在mid往左的区域中（包含mid位置），因此这种情况下更新right为mid或者mid+1——此时
     *      有两种方案。
     *          说明：上面提到的"最后一个数"，在代码中可以使用 nums[nums.length-1]，也可以使用
     *      nums[right]，当然使用 nums[right] 泛化性更好！
     *
     * 【推荐解法】解析的话就看官方的
     *      1. 解法1。————多使用一个变量，存储遇到过的最小值，这样的写法与"寻找插入位置"题目的代码类似，见
     *   方法 findMin
     *      2. 推荐使用解法2，见方法 findMin_01
     *      3. 这个题最好选择“mid位置和右边界的位置比较”，强行写成和左边界比较也是可以的，但是必须添加额外
     *   的判断逻辑，见方法 findMin_02。“额外判断逻辑”就是部分整体有序的时候直接返回 nums[left]，如何理解
     *   这个“额外的特殊逻辑”
     * */
    /*解法1（不推荐）。
    *    两种解法最大的区别在于：当找到小于的数时，右边界right更新到哪里的问题————
    *       解法1中会把right更新为mid-1，但是此时可能错过最小值，mid位置可能就是最小值。。因此使用一
    * 个变量记录；注："while(left <= right)"，因此会二分所有的位置 且 在这个过程记录答案可能的位置，
    * 见方法 findMin
    *       解法2中right不会更新为mid-1（防止错过最小值），但是循环的条件就不能带“=”了（带等于会导致
    * 死循环）。见方法 findMin_01*/
...
//    }
    ...
    * */
    public int findMin_02(int[] nums) {
        ...
        while (left < right) {
            /**重要：如果和左边界比较，需要加这个if逻辑————如果当前区间已经有序，left就是最小值*/
