...
public class _16tricks {
    ...
    * */
     ...
     *  明这个位置已经有对应的数了，如果下次哪个数对应的位置是负数，说明这个数就重复了
    ...
    public int findDuplicate(int[] nums) {
        ...
        * 针开始的位置是一样的”，如果不先走直接判断就错了。*/
        do{ /**err：这种写法必须使用“do-while循环”。do{}while();这样的循环方法，循环体至少会执行一次。第一次执行完才会判断while条件*/
            slow = nums[slow];
            fast = nums[nums[fast]];
        }while (slow!=fast);
    ...
    /* 下面的写法也是对的~~ chatgpt给出的是下面的这种形式，有什么区别？
...
必须 原地 修改，只允许使用额外常数空间。
     ...
     * 【如果有下一个更大的排列，则做法分为三步】
     *      step1. 从最后一个数倒着找，找到第一次升序的位置。即数组元素满足nums[i]<nums[i+1]。【注
     * 意】这里必须找出严格小于的，等于的话不行！！
     *      step2. 从最后一个数倒着找，找第一个大于nums[i]的数。。【注意】这里一定会找到，因为
     *  由第一步可以知道最起码nums[i+1]是大于nums[i]的。
     *      step3. 倒序"从i+1位置往后的数"。。。【注意】此时i+1往后所有数一定是降序的，因为根据
     *  第一步原来就是降序的，然后第2步将一个数换成了比它小但比后面数大的nums[i]，因此此时还
     *  是严格降序的
     *      【如果没有下一个更大的排列，说明原始数组就是最大的排列，因此逆序整个数组】
     *      【见方法 nextPermutation_2。如果flag初始化为-1，只需要翻转flag+1到数组结束的数，兼顾
     *   了全局有序的情况】
     * */
