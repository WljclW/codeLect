...
//    }
     ...
     */
    /**
     【解题思路】删除二叉搜索树中的一个节点，分为两大步：
        一、找到要删除的节点
              1. 如果key<root.val，说明需要去root的左子树查找并删除；
              2. 如果key>root.val，说明需要去root的右子树查找并删除；
              3. 否则说明key==root.val，此时执行删除，见二
        二、具体要删除时，根据该节点的处境分为几种情况————
...
//            return deleteNode(root.right, minNode.val);
    ...
    public boolean isMatch(String s, String p) {
        ...
        /*step1：初始化包含两个重要步骤。
            ...
            ②第一行的初始化：如果p串是”a*“或者”a*b*“之类的情况————即下面的for循环处理
            【补充说明】第一列不用初始化操作，因为第一列代表p是空串，除了dp[0][0]串s就不是空的了。匹配串p是空，但是s不
          是空，因此匹配的结果都是false，所以dp[1][0]、dp[2][0]、dp[3][0].....都是false即boolean数组的默认值*/
        dp[0][0] = true; //对应的是step1的①
        /*for循环对应step1的②。为什么i从2开始，因为*最早会出现在索引为1的位置（隐含说法：匹配串默认是合法的，不会出
        现p的开始就是*的情况），dp[][2]计算的就是匹配串的0位置&1位置的匹配*/
        for (int i = 2; i <=n; i++) {
            if (p.charAt(i-1)=='*'){
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                ...
                /*情况1：如果p的”j-1“位置是'.'或者等于sc，则取决于前面位置的匹配*/
                if (pc=='.'||sc==pc){
                    dp[i][j] = dp[i-1][j-1];
                }else if (pc=='*'){ /*情况2：p的”j-1“位置是‘*’的时候，需要看它前面位置的字符即”p.charAt(j-2)“————与44题的区别*/
                    /**如果前面的字符匹配了0次。
                     由于这个题默认p串是合法的，因此直接取就好，不用判断”j-2“是否越界。
                     */
                    dp[i][j] = dp[i][j-2];
                    /**如果前面的字符匹配了1次 或者 多次。。。。
                     疑问：”多次“是如何实现匹配的？？*/
...
子序列定义为：不改变剩余字符顺序的情况下，删除某些字符或者不删除任何字符形成的一个序列。
     */
    /**
     1. DP数组的含义：设 dp[i][j] 表示字符串 s[i..j]（从 i 到 j 的子串，闭区间）中，最长回文子序列的长度。
     2. 状态转移方程：
            情况1：如果i、j位置的字符相同，则dp[i][j] = dp[i+1][j-1] + 2。含义：等于中间部分子串的最长回文子
        序列 再加2；
            情况2：如果i、j位置的字符不相同，则dp[i][j] = max(dp[i+1][j], dp[i][j-1])。含义：首尾已经不相同
        了，此时要么不要尾，要么选择不要首，在剩下的子串中求结果
     3. 初始化：i和j相等 的时候，子串的长度是1，“最大回文子序列”的长度是1；
               i>j 的时候压根就不是有效的子串，因此初始化为0。可省略
     4. 遍历顺序的确定：
            ①当i和j位置字符相等的时候，因为 dp[i][j] 依赖于 dp[i+1][j-1]———— i的计算方向必
        须是从大到小的，这一步不能确定出j的遍历方向。。。计算第 i 行 需要依赖于 第 i+1 行 的信息
            ②当i和j位置字符不相等的时候，此时 dp[i][j] 会依赖于 dp[i+1][j]、dp[i][j-1]————
        即会依赖于下一行的同一位置，因此要先计算出下一行的值，即i的方向是从大到小；也会依赖于同一行前
        一个位置的数，因此同一行计算的时候需要先计算前面的数，从前往后依次计算。
            综上，行的顺序是从下往上计算每一行的元素、列的顺序是从前往后计算
    【关键总结】
           1. dp 的问题中，
            ①基本上都是“从第0行依次往后面的行计算的”，几乎所有的题目都是这种顺序，这些题目的区分度在
        于：同一行内的计算顺序是什么？
            ②这个题、“鸡蛋掉落的最短路径和”，印象中就这两个题目不一样，要从“最后一行倒着往上面的行研
        究”
     */
    ...
    */
    ...
    public int longestPalindromeSubseq(String s) {
        ...
        */
        ...
        /*step4：因为求解的是s中最长的回文子序列长度。因此返回dp[0][n-1]—————“代表[0....n-1]这个子串中
            最长回文子序列的长度”*/
        return dp[0][n-1];
    }
...
//    }
    ...
    */
    ...
    public boolean isMatch_(String s, String p) {
        ...
        for (int i = 1; i <= m; i++) {
            Arrays.fill(cur,false); /**err：“通配符匹配”题目滚动数组中，每一轮必须对cur显式的重置*/
            cur[0] = false; //可以省略。dp[i][0]表示s非空但是p是空的，因此必然是false
            for (int j = 1; j <= n; j++) {
                char sc = s.charAt(i - 1);
                char pc = p.charAt(j - 1);
                if (pc==sc||pc=='?'){
                    ...
                    /**验证下面的两种写法有没有区别。两种写法都是可行的！！*/
//                    cur[j] |= cur[j-1];
//                    cur[j] |= prev[j];
                    cur[j] = cur[j-1] || prev[j]; //用上面的2行代替这一行也是🆗的
                }else { //由于默认值就是false，因此else也可以省略
                    cur[j] = false;
                }
            }
            boolean[] tmp = prev;
        ...
        for (int i = 1; i <= m; i++) {
            ...
            for (int j = 1; j <= n; j++) {
                ...
                */
                if (sc==pc||pc=='?'){
                    /**我们从根本上看——
                     dp[j] 在进入内层循环时，存的其实是上一行的 dp[i-1][j]。然后你又用 |= 去“叠加”新的
                     状态，会把旧的匹配信息留了下来。但是这里应该是要“自立门户”的*/
                    dp[j] = prev; /**err：这里不能写成“dp[j] |= prev”*/
                } else if (pc=='*') {
                    dp[j] |=  dp[j-1];
                }else{   /**err：这里的else分支不能省略*/
                    /**为什么else分支不能省略？？
                     如果都不满足（即 pc 既不是 *、也不是 ?、也不等于 sc），你没有给 dp[j] 赋值，它就保
                     留了上一次循环的旧值（上一行的状态）！这也是“状态污染”的典型来源。
                     */
                    dp[j]=false;
                }
    ...
    /**【多源BFS】
     ...
     【思想】整体的思想类似于力扣”课程表“。。。。。这种题目往往需要使用队列，并且整体的思路类似如下：
        ...
        置，举个例子，比如”课程表“中没有前置课程的课程即入度为0的课程（即不需要额外的操作就能直接学习的课程）、再比如这个
        题目中腐烂的橘子（不需要额外的操作即可进行扩散）
            step2：①只要满足某个要求，就不断的从队列弹出东西。举例子：比如”课程表“题目中只要队列中还有东西即当前可以直接
        学习的课程（这些课程满足要么它没有前置课程，要么它所有的前置课程已经学习完成）；再比如本题中只要队列不为空，就不停的
        弹出元素进行扩散腐烂过程，因为队列中的是已经腐烂的位置可以继续感染下去（并且每一轮循环的时候，队列中的位置可以并行的
        进行腐烂感染）。
                  ②操作①以后会发现某些位置变得也符合要求了，因此加入到队列。比如：”课程表“题目中某一个课程mm的所有前置课
        程都学习完成了（表现为课程mm的入度变为0），该课程加入到队列。再比如这个题中某个位置的橘子由不腐烂变成腐烂了，该
        位置加入到队列（代表下一轮就可以从这个位置向四周继续感染了）。
     【为什么叫做"多源"？？】
            1. 含义：因为任何一个时刻，任何一个腐烂的橘子都可以同时向四周扩散（程序中的表现就是：每一轮循环进行前，队列中的所
        有位置都能弹出来，向四周开始扩散）！因此时间增加1单位时，每一个腐烂的橘子都同时向四周扩散，很明显起点是多个。
            2. 代码中是如何体现的？
        ...
        多只会进一次 queue，弹出后再也不会进入queue了————下一轮直接从被感染的位置开始感染
     */
...
//            minute++;
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int[] dir : dirs) {
                    ...
                    /*做三个事：如果①得到的某个方位不越界，并且②新位置的橘子是好的橘子（因为0是空格，1才是新鲜的橘子），就让
                    它变腐烂（即grid对应位置的值变为2）；新鲜橘子的数量-1；将腐烂橘子的位置添加进queue*/
                    if (x >= 0 && x < m && y >= 0 && y < n && grid[x][y] == 1) {
                        grid[x][y] = 2;
                        fresh--;
                        hasRot = true; /**err：更新变量，表示本轮有新鲜的橘子变腐烂了*/
...
//            minute++;
    /**err2：不能使用for循环来出队列中的位置信息。。。因为在poll()的时候，碰到变腐烂的橘子还需要把对
     应的位置添加到 queue，在删除的同时不允许修改，否则就是下面的报错，报错信息的关键字“ConcurrentModificationException”。
...
//        }
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                ...
                1、2、.....j-1”这个子串 可以拼成s3的前面的子串；②并且s1的第i个字符(即当前研究的字符即index=i-1位置
                的字符)和s3当前研究的字符是一样的，因此dp[i][j]是true；
