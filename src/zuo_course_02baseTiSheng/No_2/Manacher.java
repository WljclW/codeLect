...
public class Manacher {
    /**
     *   该函数会将形参的str预处理为manachaer需要的格式————以'#'开始，'#'结束，中间每隔一个字符插入一个'#'。
     * */
    public static char[] manacherString(String str){
        ...
        int index = 0;
        /*
         *  一种巧妙的实现：奇数位置放"#",偶数位置放实际的数。如何判断一个数是否是奇数？？？与1进行按位想与。
         * */
    ...
    public static int maxLcpsLength(String str){
        if (str == null || str.length()==0)
            return 0;
        char[] charArr = manacherString(str);    //根据初始字符串str获得一个预处理后的字符串数组。。12345-》#1#2#3#4#5#
