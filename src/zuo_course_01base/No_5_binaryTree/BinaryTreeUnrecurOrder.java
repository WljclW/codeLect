...
public class BinaryTreeUnrecurOrder {
    /**
     * 非递归先序遍历：
     *    流程：先将头节点入栈。然后只要栈不是空，就弹出节点并打印节点的值；同时对这个节点操作：如果右节点不是空则右节点入栈；如果左节点不是空则
    ...
    /**
     * 非递归后序遍历的思路：
     *    两个栈。先将头节点入栈1。然后依次执行下列操作：
     *    只要栈1不是空，弹出一个节点，将这个弹出的结果压入栈2，同时如果这个节点的左孩子不是空，则将左孩子入栈1；如果
     * 这个节点的右孩子不是空，则将右孩子入栈1.
     *    最后所有的节点都在栈2中，然后依次弹出栈2的元素即为后序遍历的结果
     * 非递归的后序遍历：
